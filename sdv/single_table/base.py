...
from sdv.unlearning.influence_tracker import InfluenceTracker
from sdv.unlearning.forget_engine import ForgetEngine
from sdv.unlearning.verifier import UnlearningVerifier
import datetime
import functools
import inspect
import logging
...
# initialize unlearning components
    ...
    # ----- machine unlearning setup -----
        if self.enable_unlearning:
            self._influence_tracker = InfluenceTracker()
            self._forget_engine = ForgetEngine()
            self._verifier = UnlearningVerifier()

            self._influence_map = self._influence_tracker.compute(data)
...
class BaseSingleTableSynthesizer(BaseSynthesizer):
    ...
    def forget(self, row_id):
        ...
        if row_id not in self._influence_map:
            raise ValueError("Row not found.")

        influence = self._influence_map[row_id]

        # Update model parameters
        self._forget_engine.remove(self, influence)

        del self._influence_map[row_id]

    def verify_unlearning(self, row_id):

        import numpy as np
        import pandas as pd

        if row_id not in self._data.index:
            raise ValueError("Row not found")
        ...
        for col in self._data.columns:
            if self._data[col].dtype == "object":
                numeric_row[col] = self._data[col].astype("category").cat.codes.loc[row_id]

        patient_vector = np.array(numeric_row.values, dtype=float)

        return self._verifier.verify(patient_vector)    
